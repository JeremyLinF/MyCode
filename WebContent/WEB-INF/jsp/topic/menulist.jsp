				...
				<div class="">
					...
					<div class="r_navigation">
						您现在的位置： <a href="<c:url value="/"/>"><f:message
							...
							href="<c:url value="/topic/menu.do?id=${menuEntity.id}"/>"><c:out
								value="${menuEntity.name}" /></a>
					</div>
					<table width="100%" border="0" cellpadding="0" cellspacing="0"
						style="border: 1px solid #a8d7f1">
						<tr>
							<td align="left" width="70%" height="20">
								<div class="class_page_topic">
									<span class="pagecss"> <jsp:include
											page="/WEB-INF/jsp/pub/page2.jsp" />
									</span>
								</div>
							</td>
							<td align="right" height="20"><input type="button"
								value="发帖"
								onclick="location='<c:url value="/master/topic.do?m=addTopic"/>';" />
							</td>
						</tr>
					</table>
					<table width="100%" border="0" cellpadding="0" cellspacing="1"
						...
						<tr>
							<td width="60%" align="left" height="30"
								style="padding-left: 6px; color: #ffffff; font-weight: bold;">标题</td>
							<td width="15%" align="center"
								style="color: #ffffff; font-weight: bold;">发帖人</td>
							<td width="10%" align="center"
								style="color: #ffffff; font-weight: bold;">回复</td>
							<td width="15%" align="center"
								style="color: #ffffff; font-weight: bold;">最后更新</td>
						</tr>
						<c:choose>
								...
								<c:forEach items="${topicList}" var="topic" varStatus="vs">
									<tr>
										<td align="left" height="40"
											bgcolor="${vs.index%2==1?'#e8e9f9':'#ffffff'}"
											style="padding-left: 6px;"><a
											href="<c:url value="/topic/topic.do?id=${topic.id}"/>"><c:out
													value="${topic.title}" /></a></td>
										<td align="center"
											bgcolor="${vs.index%2==1?'#e8e9f9':'#ffffff'}"><c:out
												value="${topic.userEntity.username}" />
											<p>
												<f:formatDate value="${topic.pubtime}" pattern="MM-dd HH:mm" />
											</p></td>
										<td align="center"
											bgcolor="${vs.index%2==1?'#e8e9f9':'#ffffff'}"><c:out
												value="${topic.replyNum}" default="0" /></td>
										<td align="center"
											bgcolor="${vs.index%2==1?'#e8e9f9':'#ffffff'}"><c:out
												value="${topic.editUser}" />
											<p>
												<f:formatDate value="${topic.editTime}"
													pattern="MM-dd HH:mm" />
											</p></td>
									</tr>
								</c:forEach>
