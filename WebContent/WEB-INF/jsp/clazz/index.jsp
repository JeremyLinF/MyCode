		...
		<div id="main_bg">
			<div id="main_right">
				<div id="main_right_box">
					...
					<div class="r_ad"
						style="border: 1px solid #A6C7CC; height: 78px; background: url(http://localhost:9080/classnet/images/leftboxContentBg.jpg)">
						<div style="padding-top: 10px; text-align: center">
							学科一级目录: <select name="pmenuId" id="pmenuId" style="width: 100px;"
								onchange="changemenu(this.value)">
								<option value="0">请选择</option>
								<c:forEach items="${menuList}" var="menu">
									<option value="${menu.id}"><c:out value="${menu.name}" /></option>
								</c:forEach>
							</select> &nbsp; 学科二级目录: <select name="menuId" id="menuId"
								style="width: 100px;">
								<option value="0">请选择</option>
							</select> &nbsp; 文件类型: <select name="ft" id="ft" style="width: 100px;">
								<option value="0">请选择</option>
								<c:forEach items="${fileTypeList}" var="ft">
									<option value="${ft.id}"><c:out value="${ft.name}" /></option>
								</c:forEach>
							</select>
						</div>
						<div style="padding-top: 10px; text-align: center">
							<input type="text" size="70" name="key" id="key"
								style="height: 20px;" /> <input type="button" value="搜索"
								style="width: 60px;" onclick="search()" />
						</div>
					</div>
						...
						<c:forEach items="${fileTypeList}" var="ft">
							<div class="childclass_main_box">
								<div class="childclass_title">
									<div class="more">
										<a href="<c:url value="/clazz/search.do?ftId=${ft.id}"/>">[更多]</a>
									</div>
									<a href="<c:url value="/clazz/search.do?ftId=${ft.id}"/>"><c:out
											value="${ft.name}" /></a>
								</div>
								<div class="childclass_content">
									<ul class="listStyle1">
										<c:forEach items="${ft.clazzList}" var="clazz">
											<li><a
												href="<c:url value="/clazz/view.do?id=${clazz.id}"/>"><c:out
														value="${clazz.name}" /></a>
											<f:formatDate value="${clazz.pubtime}" pattern="MM-dd" /></li>
										</c:forEach>
									</ul>
						...
						<dl>
							...
							<dd class="c_content">
								...
								<div class="a_photo_list">
									<c:forEach items="${newClazzList}" var="nc">
										<li>
											<div class="pe_u_thumb">
												<a href="<c:url value="/clazz/view.do?id=${nc.id}"/>"
													target="_blank"><img
													src="<f:message key="upload_http_path"/>/images/${nc.img}"
													width="160" height="120" border="0"></a>
											</div>
											<div class="pe_u_thumb_title">
												<a href="<c:url value="/clazz/view.do?id=${nc.id}"/>"
													target="_blank"><c:out value="${nc.name}" /></a> <br />
											</div>
										</li>
									</c:forEach>
									<div class="clearbox"></div>
