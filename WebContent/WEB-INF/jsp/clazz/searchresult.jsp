					...
					<div id="main_right_box">
						...
						<div class="r_ad"
							...
							<div style="padding-top: 10px; text-align: center">
								学科一级目录: <select name="pmenuId" id="pmenuId"
									style="width: 100px;" onchange="changemenu(this.value)">
									<option value="0">请选择</option>
									<c:forEach items="${menuList}" var="menu">
										<option value="${menu.id}"
											${menu.id==param.pmId?'selected=selected':''}><c:out
									...
									<c:if test="${not empty childMenuList}">
										<c:forEach items="${childMenuList}" var="cm">
											<option value="${cm.id}"
												${cm.id==param.mId?'selected=selected':''}><c:out
													value="${cm.name}" /></option>
										</c:forEach>
									</c:if>
									...
									<c:forEach items="${fileTypeList}" var="ft">
										<option value="${ft.id}"
											${ft.id==param.ftId?'selected=selected':''}><c:out
												value="${ft.name}" /></option>
									</c:forEach>
								</select>
							</div>
							<div style="padding-top: 10px; text-align: center">
								<input type="text" size="70" name="key" id="key" value="${key}"
								...
								<dt class="c_title">
									<div class="more">
										<div class="childclass_title">
											<div class="more">
												<a href="http://dvd.boxuegu.com/map/22.html">[更多]</a>
											</div>
										</div>
									</div>
									搜索结果
										...
										<c:forEach items="${clazzList}" var="clazz">
											<li>
												<div class="pe_u_thumb">
													<a href="<c:url value="/clazz/view.do?id=${clazz.id}"/>"
														target="_blank"><img
														src="<f:message key="upload_http_path"/>/images/${clazz.img}"
														width="160" height="120" border="0"></a>
												</div>
												<div class="pe_u_thumb_title">
													<a href="<c:url value="/clazz/view.do?id=${clazz.id}"/>"
														target="_blank"><c:out value="${clazz.name}" /></a> <br />
												</div>
											</li>
										</c:forEach>
