...
<div style="padding-right: 20px;">
	<span class="txt"> <c:if test="${PageInfo.prev}">
			...
			<c:otherwise>
				<c:choose>
					...
					<c:when
						test="${(PageInfo.total_page-PageInfo.page)>=0 && (PageInfo.total_page-PageInfo.page) <5}">
						<c:set var="firstPage" value="${PageInfo.total_page-4}"
							scope="page" />
						<c:set var="endPage" value="${PageInfo.total_page}" scope="page" />
					</c:when>
					...
					<c:when test="${(PageInfo.total_page-PageInfo.page) >5}">
						<c:set var="firstPage"
							value="${(PageInfo.total_page-PageInfo.page)+1}" scope="page" />
						<c:set var="endPage" value="${PageInfo.total_page}" scope="page" />
					</c:when>
		...
		</c:choose> <c:if test="${firstPage>0}">
		...
		</c:if> <c:if test="${PageInfo.next}">
			<a href="<c:url value="${PageInfo.url}${PageInfo.page+1}"/>">下一页</a>
		</c:if> <strong>每页<c:out value="${PageInfo.page_size}" />条,共<c:out
				value="${PageInfo.total_page}" />页
	</strong>
	</span>
