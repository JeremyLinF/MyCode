								...
								<dd>
									<ul>
										<c:forEach items="${tjnews}" var="news">
											<li><a
												href="<c:url value="/clazz/view.do?id=${news.id}"/>"><c:out
														value="${news.title}"></c:out></a></li>
										</c:forEach>
									</ul>
								</dd>
					...
					<div class="c_main">
						...
						<div class="r_navigation">
							您现在的位置：<a href="<c:url value="/"/>"><f:message
								...
								href="<c:url value="/news/menu.do?id=${menuEntity.id}"/>"><c:out
						...
						<div class="childclasslist_box">
							...
							<ul class="listStyle1">
								<c:forEach items="${newsList}" var="news">
									<li><a href="<c:url value="/news/see.do?id=${news.id}"/>"
										target="_blank"><c:out value="${news.title}" /></a>
									<f:formatDate value="${news.pubtime}" pattern="MM-dd" /></li>
								</c:forEach>
							</ul>
						</div>
						<!-- 子栏目列表信息列表结束 -->
						<div class="class_page">
							<span id="pe100_page_内容信息列表_普通式" class="pagecss"> <jsp:include
