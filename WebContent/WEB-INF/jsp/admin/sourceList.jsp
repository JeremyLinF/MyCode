		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td class="left" valign="top"><jsp:include
						page="pub/left_source.jsp"></jsp:include></td>
				<td valign="top">
					<div class="middle">
						<table width="99%" border="0" cellpadding="0" cellspacing="1"
							...
							<tr>
								<td height="30" bgcolor="#ffffff" align="left"
									style="padding-left: 10px;">选择目录: <select name="menuId"
									id="menuId" onchange="changeMenu(this.value)">
										<option value="0">全部</option>
										<c:forEach items="${menuList}" var="menu">
											<option value="${menu.id}"
												${param.menuId==menu.id?'selected=selected':''}><c:out
													value="${menu.name}" /></option>
										</c:forEach>
								</select>&nbsp;&nbsp; 关键字: <input type="text" name="key" id="key"
									value="${key}" /> &nbsp;&nbsp; <input type="button" value="查询"
									onclick="search()" /> &nbsp;&nbsp;&nbsp;&nbsp; <input
									type="button" value="添加"
									onclick="location='<c:url value="/admin/source/source.do?m=add"/>';" />
								</td>
							</tr>
						</table>
					</div>
					<div id="c01">
						<table width="99%" border="0" cellpadding="0" cellspacing="1"
							...
							<tr>
								...
								<td width="45%" height="30" bgcolor="#e5edfa"><div
										align="center" class="STYLE3">名称</div></td>
								<td width="15%" height="30" bgcolor="#e5edfa"><div
										align="center" class="STYLE3">目录</div></td>
								<td width="15%" height="30" bgcolor="#e5edfa"><div
										align="center" class="STYLE3">文件大小</div></td>
								<td width="15%" height="30" bgcolor="#e5edfa"><div
									...
									<c:forEach items="${sourceList}" var="source" varStatus="vs">
										<tr>
											<td height="30" bgcolor="#FFFFFF" align="center"><input
												type="checkbox" name="sourceId" value="${source.id}" />&nbsp;<c:out
													value="${vs.index+1}" /></td>
											<td height="30" bgcolor="#FFFFFF" align="center"
												id="menu${source.id}"><a
												href="<f:message key="upload_http_path"/>/files/${source.filename}"><c:out
														value="${source.name}" /></a></td>
											<td height="30" bgcolor="#FFFFFF" align="center"
												id="menu${source.id}"><c:out
													value="${source.sourceMenu.name}" /></td>
											<td height="30" bgcolor="#FFFFFF" align="center"
												id="menu${source.id}"><c:out value="${source.fileSize}" />
											</td>
											<td height="30" bgcolor="#FFFFFF" align="center"><a
												href="<c:url value="/admin/source/source.do?m=edit&id=${source.id}"/>">修改</a>
												<a href="javascript:del(${source.id})">删除</a></td>
										</tr>
									</c:forEach>
								</c:when>
								<c:otherwise>
