...
function edit_submit(id){
	...
	$.ajax({
		type:"post",
		url:"<c:url value="/admin/topic/topicMenu.do"/>",
...
function del(id){
	if(id!=0){
		location='<c:url value="/admin/topic/topicMenu.do?m=del&id="/>'+id;
	}
	else{
		var array = document.getElementsByName("menuId");
		var idstr="";
		...
		if(idstr){
			location='<c:url value="/admin/topic/topicMenu.do?m=del&ids="/>'+idstr;
		}
		else{
			alert("您还未选择");
		}
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td class="left" valign="top"><jsp:include
						page="pub/left_topic.jsp"></jsp:include></td>
				<td valign="top">
					<div class="middle">
						<form action="<c:url value="/admin/topic/topicMenu.do?m=add"/>"
