					...
					<div id="c01">
						<dl>
							<dt style="padding-right: 20px;">
								分类： <select id="menuId" name="menuId" style="width: 100px;"
									onchange="changeMenu(this.value)">
									<option value="0">全部</option>
									<c:forEach items="${menuList}" var="menu">
										<option value="${menu.id}"
											${menu.id==menuId?'selected=selected':''}><c:out
									...
									<c:forEach items="${topicList}" var="topic" varStatus="vs">
										<tr>
											<td height="30" bgcolor="#FFFFFF" align="center"><input
												type="checkbox" name="topicId" value="${topic.id}" /> <c:out
													value="${vs.index+1}" /></td>
											<td height="30" bgcolor="#FFFFFF" align="left"
												style="padding-left: 10px;"><a
												href="<c:url value="/topic/topic.do?id=${topic.id}"/>"
												target="_blank"><c:out value="${topic.title}" /></a></td>
											<td height="30" bgcolor="#FFFFFF" align="center"><c:out
													value="${topic.userEntity.username}" /></td>
