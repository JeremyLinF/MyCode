...
function edit_submit(id){
	...
	$.ajax({
		type:"post",
		url:"<c:url value="/admin/clazz/clazzMenu.do"/>",
...
function del(id){
	if(id!=0){
		location='<c:url value="/admin/clazz/clazzMenu.do?m=del&id="/>'+id;
	}
	else{
		var array = document.getElementsByName("menuId");
		var idstr="";
		...
		if(idstr){
			location='<c:url value="/admin/clazz/clazzMenu.do?m=del&ids="/>'+idstr;
		}
		else{
			alert("您还未选择");
		}
...
function changeclazzmenu(pid){
	var url='<c:url value="/admin/clazz/clazzMenu.do?m=list"/>';
	if(pid!=0){
		url+='&pid='+pid;
	}
	location=url;
	...
	<div class="content">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td valign="top">
					<div class="middle">
						<form action="<c:url value="/admin/clazz/clazzMenu.do?m=add"/>"
							method="post" style="margin: 0px;" onsubmit="return check(this)">
							<table width="99%" border="0" cellpadding="0" cellspacing="1"
								bgcolor="#dae2e5">
								<tr>
									<td width="30%" height="30" bgcolor="#ffffff" align="center">
										父目录: <select name="pid" id="pid" style="width: 150px;"
										onchange="changeclazzmenu(this.value)">
											<option value="0">无</option>
											<c:forEach items="${topMenuList}" var="topMenu">
												<option value="${topMenu.id}"
													${param.pid==topMenu.id?'selected=selected':''}><c:out
														value="${topMenu.name}" /></option>
											</c:forEach>
									</select>
									</td>
									<td width="50%" height="30" bgcolor="#ffffff" align="center">
										目录名称: <input type="text" name="name" id="name" />
									</td>
						...
						<table width="99%" border="0" cellpadding="0" cellspacing="1"
							...
							<tr>
								...
								<td width="60%" height="30" bgcolor="#e5edfa"><div
										align="center" class="STYLE3">名称</div></td>
								<td width="20%" height="30" bgcolor="#e5edfa"><div
									...
									<c:forEach items="${menuList}" var="menu" varStatus="vs">
										<tr>
											<td width="7%" height="30" bgcolor="#FFFFFF" align="center">
												<input type="checkbox" name="menuId" value="${menu.id}" />&nbsp;<c:out
													value="${vs.index+1}" />
											</td>
											<td width="10%" height="30" bgcolor="#FFFFFF" align="center"
												id="menu${menu.id}"><c:out value="${menu.name}" /></td>
											<td width="10%" height="30" bgcolor="#FFFFFF" align="center">
												<a id="edit${menu.id}"
												href="javascript:edit(${menu.id},'${menu.name}')">修改</a> <a
												id="edit_submit${menu.id}"
												href="javascript:edit_submit(${menu.id})"
												style="display: none">保存</a> <a
												href="javascript:del(${menu.id})">删除</a>
											</td>
										</tr>
									</c:forEach>
								</c:when>
