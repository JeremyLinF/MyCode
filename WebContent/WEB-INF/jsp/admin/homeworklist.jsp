							...
							<table width="99%" border="0" cellpadding="0" cellspacing="1"
								...
								<tr>
									<td height="30" bgcolor="#ffffff" align="center"><input
										type="hidden" name="m" value="list" /> 关键字:<input type="text"
										name="key" value="${key}" /> 用户名:<input type="text"
										name="username" value="${username}" /> <input type="hidden"
										name="titleId" value="${titleEntity.id}" /> <input
					...
					<div id="c01">
									...
									<c:forEach items="${homeworkList}" var="hw" varStatus="vs">
										<tr>
											...
											<td bgcolor="#FFFFFF" align="center"><a
												...
												target="_blank"
												href="<c:url value="/admin/homework.do?m=download&id=${hw.id}"/>">下载</a>
											</td>
										</tr>
									</c:forEach>
								</c:when>
						...
						<p>
							<input type="checkbox" onclick="selectAll(this)" /> 全选 <input
								type="button" value="删除" class="shanchu" onclick="del(0)" /> <input
								type="button" value="返回题目列表"
								onclick="location='<c:url value="/admin/homework.do?m=titleList"/>';" />
						</p>
					</div> <br /> <br />
				</td>
			</tr>
