	...
	<div class="content">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td valign="top">
					<div class="middle">
						<form action="<c:url value="/admin/clazz/fileType.do?m=add"/>"
									...
									<c:forEach items="${fileTypeList}" var="ft" varStatus="vs">
										<tr>
											<td width="7%" height="30" bgcolor="#FFFFFF" align="center">
												<input type="checkbox" name="menuId" value="${ft.id}" />&nbsp;<c:out
													value="${vs.index+1}" />
											</td>
											<td width="10%" height="30" bgcolor="#FFFFFF" align="center"
												id="menu${ft.id}"><c:out value="${ft.name}" /></td>
											<td width="10%" height="30" bgcolor="#FFFFFF" align="center">
												<a id="edit${ft.id}"
												href="javascript:edit(${ft.id},'${ft.name}')">修改</a> <a
												id="edit_submit${ft.id}"
												href="javascript:edit_submit(${ft.id})"
												style="display: none">保存</a> <a
												href="javascript:del(${ft.id})">删除</a>
											</td>
										</tr>
									</c:forEach>
								</c:when>
