...
<title>后台管理--<c:choose>
		<c:when test="${empty clazzEntity}">发布课程</c:when>
		<c:otherwise>编辑课程</c:otherwise>
	</c:choose></title>
	...
	<div class="content">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td valign="top">
					<div class="right">
						<form action="<c:url value="/admin/clazz/add.do?m=doadd"/>"
							...
							<table width="80%" border="0" cellpadding="0" cellspacing="1"
										...
										onchange="changeMenu(this.value)">
											...
											<c:forEach items="${menuList}" var="menu">
												<option value="${menu.id}"
													${clazzEntity.clazzMenu.parentMenuEntity.id==menu.id?'selected=selected':''}><c:out
														value="${menu.name}" /></option>
											</c:forEach>
									</select> <select name="menuId" id="menuId" class="sheng">
											<c:if test="${not empty clildMenuList}">
												<c:forEach items="${clildMenuList}" var="cm">
													<option value="${cm.id}"
														${cm.id==clazzEntity.clazzMenu.id?'selected=selected':''}><c:out
								...
								<tr>
									...
									<td height="28" bgcolor="#FFFFFF"><input type="text"
										name="name" id="name" size="60" value="${clazzEntity.name}"
										maxlength="25" />
									<p>请输入课程名称</p></td>
								</tr>
								<tr>
									...
									<td height="28" bgcolor="#FFFFFF"><input type="text"
										...
										value="<c:out value="${clazzEntity.author}" default="admin"/>"
										maxlength="10" />
									<p>请输入课程作者</p></td>
								</tr>
								<tr>
									...
									<td height="28" bgcolor="#FFFFFF"><input type="text"
										name="school" value="${clazzEntity.school}" maxlength="10" />
									<p>请输入作者单位</p></td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td height="28" bgcolor="#FFFFFF"><select
										name="filetypeId" id="filetypeId">
											<option value="0">请选择文件类型</option>
											<c:forEach items="${fileTypeList}" var="ft">
												<option value="${ft.id}"
													${clazzEntity.fileType.id==ft.id?'selected=selected':''}><c:out
								...
								<tr>
									<tr>
										<td width="15%" height="28" align="right" bgcolor="#edf1f7"><c:if
												test="${empty clazzEntity}">
												<span>*</span>
											</c:if>课程文件：</td>
										<td height="28" bgcolor="#FFFFFF"><input type="file"
											name="file" id="file" />
