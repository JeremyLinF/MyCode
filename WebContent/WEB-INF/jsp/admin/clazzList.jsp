					...
					<div id="c01">
						<dl>
							<dt style="padding-right: 20px;">
								新闻栏目： <select id="parentMenuId" name="parentMenuId"
									style="width: 100px;" onchange="changeselect()">
								...
								<c:if test="${not empty childMenuList}">
									<select id="menuId" name="menuId" style="width: 100px;"
								...
								文件类型: <select id="fileTypeId" name="fileTypeId"
									style="width: 100px;" onchange="changeselect()">
									<option value="0">全部</option>
									...
									<c:forEach items="${clazzList}" var="clazz" varStatus="vs">
										<tr>
											<td height="30" bgcolor="#FFFFFF" align="center"><input
												type="checkbox" name="clazzId" value="${clazz.id}" /> <c:out
													value="${vs.index+1}" /></td>
											<td height="30" bgcolor="#FFFFFF" align="left"
												style="padding-left: 10px;"><a
												href="<c:url value="/clazz/view.do?id=${clazz.id}"/>"
												target="_blank"><c:out value="${clazz.name}" /></a></td>
											<td height="30" bgcolor="#FFFFFF" align="center"><f:formatDate
													value="${clazz.pubtime}" pattern="yyyy-MM-dd HH:mm" /></td>
											<td height="30" bgcolor="#FFFFFF" align="center"><c:out
													value="${clazz.viewCount}" /></td>
											<td height="30" bgcolor="#FFFFFF" align="center"><select
												onchange="changestatus(${clazz.id},this.value)">
											...
											<td width="10%" height="30" bgcolor="#FFFFFF" align="center">
												<a
												href="<c:url value="/admin/clazz/clazz.do?m=edit&id=${clazz.id}"/>">修改</a>
