									...
									<c:forEach items="${userList}" var="user" varStatus="vs">
										<tr>
											<td height="30" bgcolor="#FFFFFF" align="center"><input
												type="checkbox" name="userId" value="${user.id}" /> <c:out
													value="${vs.index+1}" /></td>
											<td height="30" bgcolor="#FFFFFF" align="left"
												style="padding-left: 10px;"><c:out
													value="${user.username}" /></td>
											<td height="30" bgcolor="#FFFFFF" align="center"><c:out
													value="${user.password}" /></td>
											<td height="30" bgcolor="#FFFFFF" align="center"><c:out
													value="${user.email}" /></td>
											<td height="30" bgcolor="#FFFFFF" align="center"><c:choose>
											...
											<td width="10%" height="30" bgcolor="#FFFFFF" align="center">
												<c:choose>
													<c:when test="${user.enable}">
														<a
															href="<c:url value="/admin/user.do?m=changestatus&id=${user.id}&status=0"/>">停用</a>&nbsp;
	      		恢复
	      		</c:when>
													<c:otherwise>
	      		停用&nbsp;
	      		<a
															href="<c:url value="/admin/user.do?m=changestatus&id=${user.id}&status=1"/>">恢复</a>
													</c:otherwise>
												</c:choose>
											</td>
										</tr>
