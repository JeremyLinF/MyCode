			...
			<link rel="stylesheet"
				...
				<link rel="stylesheet"
					...
					<link rel="stylesheet"
						...
						<title>会员中心--<c:choose>
								<c:when test="${empty topicEntity}">发布主题</c:when>
								<c:otherwise>编辑主题</c:otherwise>
							</c:choose></title> <script type="text/javascript">
		function check(form){
			if(form.title.value==""){
				alert("标题不能为空");
				return false;
					...
					<div class="u_form1">
						<div style="text-align: center">
							<table width="100%" border="0" cellpadding="0" cellspacing="0">
								<tr align="center">
									<td id="TabTitle0" class="titlemouseover">发布主题</td>
								</tr>
							</table>
							<form action="<c:url value="/master/topic.do?m=doAddTopic"/>"
								method="post" onsubmit="return check(this)">
								<table class="border" id="EgvContent" style="width: 100%;"
									border="0" cellpadding="0" cellspacing="1">
									<tr class="tdbg" align="center">
										<td height="25" width="12%" align="right"
											style="padding-right: 8px">标题:</td>
										<td align="left" style="padding-left: 8px"><input
											type="text" name="title" value="${topicEntity.title}"
											size="60" maxlength="200" /></td>
									</tr>
									<tr class="tdbg" align="center">
											...
											name="menuId">
												...
												<c:forEach items="${menuList}" var="menu">
													<option value="${menu.id}"
														${topicEntity.menuEntity.id==menu.id?'selected=selected':''}><c:out
									...
									<tr class="tdbg" align="center">
										...
										<td align="left" style="padding-left: 8px; line-height: 22px">
											<textarea name="content"><c:out
													value="${topicEntity.detail}" escapeXml="false" /></textarea>
										</td>
									</tr>
									<tr class="tdbg" align="center">
										<td height="30" align="right" style="padding-right: 8px"></td>
										<td align="left" style="padding-left: 8px"><input
											type="submit" value="提交作业" /> <c:if
												test="${not empty topicEntity}">
												<input type="button" value="返回"
													onclick="javascript:history.back(-1);" />
												<input type="hidden" name="topicId"
													value="${topicEntity.id}" />
											</c:if></td>
									</tr>
								</table>
							</form>
