			...
			<link rel="stylesheet"
				...
				<link rel="stylesheet"
					...
					<link rel="stylesheet"
						...
						<title>会员中心--互动交流</title> <script type="text/javascript">
					...
					<div class="u_form1">
						<div style="text-align: center">
							<table width="100%" border="0" cellpadding="0" cellspacing="0">
								<tr align="center">
									<td id="TabTitle0" class="titlemouseover">我发布的帖子</td>
								</tr>
							</table>
							<table width="100%" border="0" cellpadding="0" cellspacing="0">
								<tr align="center">
									<td height="30" align="left">主题目录：<select name="menuId"
										onchange="changeMenu(this.value)">
											<option value="0">请选择目录</option>
											<c:forEach items="${menuList}" var="menu">
												<option value="${menu.id}"
													${menuId==menu.id?'selected=selected':''}><c:out
									...
									<c:when test="${not empty topicList}">
										<c:forEach items="${topicList}" var="topic" varStatus="vs">
											<tr class="tdbg" align="center">
												...
												<td align="left" style="font-weight: bold;"><a
													href="<c:url value="/master/"/>"><c:out
															value="${topic.title}" /></a></td>
												<td align="center"><c:out
														value="${topic.menuEntity.name}"></c:out></td>
												<td align="center"><f:formatDate
														value="${topic.pubtime}" pattern="yyyy-MM-dd HH:mm" /></td>
												<td align="center" style="font-weight: bold;"><a
													href="<c:url value="/master/topic.do?m=edit&id=${topic.id}"/>">修改</a>&nbsp;
													<a
													href="<c:url value="/master/topic.do?m=del&id=${topic.id}"/>">删除</a>
												</td>
											</tr>
										</c:forEach>
									</c:when>
